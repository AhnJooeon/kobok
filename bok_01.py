...
stat_codes = Get_Statcode()
folder_path = './Result/'  # 폴더 경로
files = os.listdir(folder_path)

print(files)  # 파일 + 폴더 이름이 모두 나옴

for stat_code in stat_codes:
    if f"ecos_{stat_code}.csv" not in files:
        Run_API(stat_code)
    else:
        print(f"[SIKP] ecos_{stat_code}.csv")